...
function fixtureRun(graphVersionId: string, id = "run-lifecycle"): AnalysisRun {
  return {
    ...
    intent: {
      ...
      confidence: 1,
      filters: {},
      meta: { schemaVersion: "1.1", source: "llm", requestId: `${id}-request`, warnings: [] },
    },
  ...
  it("persists graph, view, analysis, event, message and research session in memory", async () => {
    ...
    const run: AnalysisRun = {
      ...
      intent: {
        ...
        confidence: 1,
        filters: {},
        meta: { schemaVersion: "1.1", source: "llm", requestId: "request-1", warnings: [] },
      },
      engine: "local_algorithm",
      status: "succeeded",
  ...
  it("purges only session-owned records and preserves its formerly exclusive graph", async () => {
    ...
    const run: AnalysisRun = {
      ...
      intent: {
        ...
        confidence: 1,
        filters: {},
        meta: { schemaVersion: "1.1", source: "llm", requestId: "request-exclusive", warnings: [] },
      },
      engine: "local_algorithm",
      status: "succeeded",
      createdAt: "2026-08-10T00:00:00.001Z",
